...
Yanbai (砚白) incorporates the third-party components listed below. Each component is distributed in binary form as part of the Software and remains governed by its own license terms; nothing in the Yanbai End User License Agreement modifies or replaces those terms.

本文件列出随砚白一同以二进制形式分发的第三方组件，各组件仍适用其自身的许可条款，《砚白最终用户使用许可协议》不改变也不替代这些条款。

The list is generated from Cargo.lock and frontend/package.json for version 2.0.6. The complete license text of each component is available from that component's own distribution.
...
| x11 | 2.21.0 | MIT |
| x11-dl | 2.21.0 | MIT |
| yanbai | 2.0.6 | 见组件自身分发 |
| yanbai-core | 2.0.6 | 见组件自身分发 |
...
- HarmonyOS Sans SC (Regular, Bold): copyright Huawei Device Co., Ltd., used under the license terms distributed with the font.
- ThGrotesk (Regular, Italic, Bold, Bold Italic): designed by the Licensor and not a third-party component.
